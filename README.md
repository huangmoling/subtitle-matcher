...
- **递归扫描** —— 自动扫遍目录下**所有层级**的子目录，不用一层层点
- **认视频文件，不认文件夹** —— 一个目录里放多个视频也能各自配上字幕
- **支持 `.strm`** —— Jellyfin / Emby 的流地址占位文件同样会被识别
- **多站并行搜索** —— 同时到三个站点搜，哪个有结果就用哪个
- **跨站选优** —— 语言优先（简体 > 繁体），同语言下**选文件最大的**
- **下载失败自动退到下一个候选** —— 首选站的 CDN 挂了也不会白跑
- **自选目录** —— Windows 原生文件夹选择框，任意磁盘、任意路径
- **自动跳过** —— 视频已有同名字幕则跳过
- **自动命名** —— 保存为「视频同名.srt」（ASS 内容会存成 `.ass`），直接适配 Jellyfin / Emby / Plex
- **点列头排序** —— 每一列都能点，再点一次反向；大小默认从大到小
- **右键复制文件名** —— 在任意一行上点右键，把该视频的文件名复制到剪贴板
...
1. 点「浏览…」选媒体库目录（也可以把目录直接拖到 exe 上，或用命令行传入）
2. 勾选需要的选项
3. 点「开始匹配」

窗口里会实时显示每个视频文件的所在目录、来源、语言、大小、状态和说明。

- **排序**：点任意列头，▲/▼ 表示当前方向；同一列再点一次反向
- **复制**：在行上点右键，文件名进剪贴板
...
| 选项 | 默认 | 说明 |
|---|---|---|
| 只处理番号视频 | 开 | 只处理文件名符合番号格式的视频，避免误匹配 |
| 自动修正时间轴格式 | 开 | 把非标准时间轴改成标准 SRT |
| 已有字幕则跳过 | 开 | 该视频已有同名字幕就跳过 |
...
1. **递归**扫遍选定目录下所有层级的视频文件，跳过 `@eaDir`、`#recycle` 和以 `.` 开头的目录
2. 用**视频文件名**（去掉扩展名）作为关键字，**并行**到三个站点搜索
3. 各站解析出候选字幕，带上语言和文件大小
   - 站点没提供大小的（javzimu）会补一次 `HEAD` 请求拿到真实大小，保证比大小是公平的
4. 合并所有候选，按「语言优先（简 > 繁）→ 同语言取最大」排序
5. 从最优候选开始下载；失败就退到下一个候选（最多 3 个）
6. 保存为 `<视频文件名>.srt`，与视频放在同一目录

### 处理单位是视频文件

这一点和早期版本不同。以前是「一个文件夹 = 一个番号」，现在按视频文件来：

```
媒体库/
├── SNOS-115/
│   ├── SNOS-115.strm          → 找 SNOS-115，存成 SNOS-115.srt
│   └── SNOS-115.srt           （已存在则跳过这一条）
├── 合集/
│   └── IPZZ-508/
│       └── IPZZ-508.strm      → 嵌套两层也能扫到
└── 剧集/
    ├── ABF-353-cd1.mp4        → 各自配各自的字幕
    └── ABF-353-cd2.mp4
```

好处是：一个目录里放多集、或者目录名和视频名对不上时，都不会漏配或错配。

「已有字幕」的判断也按**视频名**来（大小写不敏感），不再看"目录里有没有字幕文件"——
否则同目录下的其它视频会被一起误跳过。

### 防误匹配

把任意文件名丢去搜索会匹配到完全无关的字幕（例如 `subtitle-matcher` 曾匹配到某部毫不相干的片子）。因此有两道防线：

- **番号格式过滤** —— 默认只处理文件名符合番号格式的视频，正则为
  `^\d{0,5}[A-Z]{2,10}[-_ ]?\d{2,6}`，覆盖 `SNOS-115`、`SSIS-001`、`259LUXU-1234`、`HEYZO-1234` 等常见写法
- **标题相关性校验** —— 搜索结果标题归一化（去符号、转大写）后必须真的包含该番号，否则丢弃

### 支持的视频格式

`.strm` `.mp4` `.mkv` `.avi` `.wmv` `.mov` `.m4v` `.ts` `.m2ts` `.mts`
`.mpg` `.mpeg` `.flv` `.webm` `.rmvb` `.rm` `.divx` `.3gp`

其中 **`.strm`** 是 Jellyfin / Emby 的流地址占位文件（内容只是一行 URL），本地媒体库里大量存在。
...
单个视频的**搜索阶段整体限时 45 秒**：某个站点卡住时，用已经拿到的候选继续往下走，而不是一起干等。
...
- 递归扫描（多层子目录、一个目录多个视频、`.strm` 识别、非视频扩展名排除、跳过 `@eaDir` 与隐藏目录）
- 「已有字幕」按视频名判断（同目录其它视频不被误跳过、大小写不敏感）
- 自然排序（`ABF-2` 排在 `ABF-10` 前面）与状态排序权重
...
- **排序用 view 映射** —— 行数据始终保持扫描顺序（下标就是进度回调的索引），另用一张 `view` 表记录显示顺序。直接对数据排序会把进度更新打到错误的行上
- **源码结构** —— `win32.go`（系统调用层）/ `gui.go`（界面）/ `source.go`（各站点解析）/ `subtitle.go`（字幕校验与规范化）/ `engine.go`（扫描、调度与选优）/ `net.go`（限速 HTTP）/ `main.go`（入口）
